#!/usr/bin/env python3
"""
train_doppler_vit.py — RepDopplerViT for Doppler spectrogram HAR.
HeadGest dataset: n_antennas=1, so each window (1,T,V) is processed directly
with no antenna expansion or fusion.
...
class DopplerDataset(Dataset):
    """Loads (1, T, V) windows saved by create_train_headgest.py.
    cache_ram=True preloads them once in __init__ so __getitem__ never hits disk."""
    ...
    def __getitem__(self, idx):
        if self._cache is not None:
            x = torch.from_numpy(self._cache[idx].copy())
        else:
            x = torch.from_numpy(self._load_file(idx))
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features: Tensor, labels: Tensor) -> Tensor:
        ...
        sim = (f @ f.T) / self.temperature

        lbl = labels.view(-1, 1)
        same_class = (lbl == lbl.T).float()
...
class RepDopplerViT(nn.Module):
    ...
    Input  : (B, 1, T, V)   — window_frames=250, vel_bins=100
    ...
    def __init__(self, n_classes: int,
        ...
        # hybrid_stage3 (Downsample x2 -> MBConv -> LinearAttn) takes
        # precedence over mobile_stage3 (LinearAttn replaced by MBConv) when both True.
        super().__init__()
        ...
        if hybrid_stage3:
            # 250-frame input: after stem+down1+down2 → 62×25
            #   down_pre[0]: 62×25 → 31×12
            #   down_pre[1]: 31×12 → 15×6
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Train RepDopplerViT on HeadGest Doppler spectrogram dataset. '
                    'n_antennas=1, no late fusion.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument('dataset_dir',
        help='Dataset folder from creating_train.py  (e.g. output/doppler/S1a)')
    p.add_argument('--activities', default=None,
        help='Comma-separated activity codes, e.g. FW,LD,LL,LR,LU,ND,SH. '
    ...
    p.add_argument('--mobile_stage3',   action='store_true',
        help='Replace stage3 LinearAttnBlock with MBConvBlock (pure XNNPACK path).')
    p.add_argument('--no_hybrid_stage3', dest='hybrid_stage3', action='store_false',
        help='Disable hybrid stage3; use plain LinearAttnBlock. '
             'Default: hybrid_stage3=True (Downsample→MBConv→LinearAttn).')
    ...
    p.add_argument('--no_simple_head',  dest='simple_head', action='store_false',
        help='Revert to original ConvBNAct head. '
             'Default: simple_head=True (GAP→BN→Dropout→Linear).')
    p.set_defaults(simple_head=True)
    p.add_argument('--no_cache_ram', dest='cache_ram', action='store_false',
        help='Disable preloading all windows into RAM (use on low-memory machines).')
...
def main():
    ...
    print('=' * 65)
    print('RepDopplerViT  —  HeadGest  (RepViT + EfficientViT, CVPR 2024)')
    print('=' * 65)
    ...
    # so the script also runs on PyTorch 1.8 and 1.12 (Xavier).
    ...
    for ep in range(1, args.epochs + 1):
        ...
        # Save on strictly better accuracy, OR equal accuracy with lower val loss.

...
Phase 3 — Few-Shot Fine-Tuning on S2a
    ...
    Trainable  : head  (+stage3 for K ≥ 10)
...
SAMPLING_HZ    = 167    # radar packets per second  (1 frame = 1/167 s ≈ 5.99 ms)
WINDOW_FRAMES  = 100    # frames per window  →  overridden at runtime via --window_frames
WINDOW_STRIDE  = 10     # stride between windows  →  overridden at runtime via --window_stride
...
class MemDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # torch.roll copies, so the cached tensor is never mutated in place.
...
def load_scenario_windows(doppler_dir: Path,
    ...
    """Load Doppler windows for a given scenario, globbing signal_{scenario}_{act}[N]_*.npz
    (N optional digit suffix). max_windows caps frames loaded per class to limit RAM."""
    label_dict: Dict = {act: idx for idx, act in enumerate(activities)}
    windows: List[np.ndarray] = []
    labels:  List[int]        = []

    for act in activities:
        # Collect frames per antenna across all numbered recording variants (C1, C2 -> C).
        ant_data: Dict[int, List[np.ndarray]] = {}
        for fpath in sorted(doppler_dir.glob(f'signal_{scenario}_{act}*_*.npz')):
            m = re.match(
                rf'signal_{re.escape(scenario)}_{re.escape(act)}\d*_(?:ant)?(\d+)\.npz',
                fpath.name)
            if m:
                ant_idx = int(m.group(1))
                ant_data.setdefault(ant_idx, []).append(
                    np.load(fpath)['doppler'].astype(np.float32))
        if not ant_data:
            print(f'  [warn] {scenario} activity {act}: no files in {doppler_dir}')
            continue
        ant_idxs = sorted(ant_data.keys())
        # Concatenate sessions along time axis, then align antennas to shortest.
        raw     = [np.concatenate(ant_data[i], axis=0) for i in ant_idxs]
        min_f   = min(d.shape[0] for d in raw)
        avail_w = (min_f - window_length) // stride_length + 1
        # Cap frames loaded to only what is needed for max_windows.
        ...
        print(f'  {scenario} [{act}]: {min_f} frames → {n_win} windows'
              f'{cap_note}  ({len(ant_idxs)} antennas)')
...
def few_shot_finetune(model: RepDopplerViT,
    ...
    supcon_weight re-clusters S2a embeddings to fix cross-class domain-shift
    ...
    frozen stages for faster inference. class_ids restricts the loss to a
    subset of the model's output classes (activities missing on this device).
    ...
    # 5. LR schedule — warmup -> cosine, via LambdaLR since LinearLR/SequentialLR
    # need torch >=1.11 (Jetson Nano JetPack 4 ships 1.8).
...
def eval_query(model: RepDopplerViT,
    ...
    """Late-fusion eval. Returns (acc, f1_macro, f1_weighted, cm, model_inf_s, n_windows),
    where model_inf_s is pure forward-pass time. class_ids restricts argmax/confusion
    matrix to a subset of model-output classes (None = use raw model outputs)."""
    model.eval()
    # batch_size=1 matches deployment and avoids OOM on shared-memory GPUs
    # (Jetson Nano 2GB) when batching many (1,360,100) windows at once.
    ...
    use_par = (device.type == 'cpu' and (os.cpu_count() or 1) >= n_antennas)
    ...
    lbls = (list(range(len(class_ids))) if class_ids is not None
            else sorted(set(targets)))
    cm = confusion_matrix(targets, preds, labels=lbls)
...
def profile_single_window(model: nn.Module,
    ...
    # Parallel per-antenna latency (CPU only): one batch-1 forward per core via
    # a thread pool, measured ~2.5x faster than one batch-n_ant call on ARM.
    # Any failure just skips this metric; the standard latency is still reported.
    par_ms: Optional[float] = None
    if device.type == 'cpu' and (os.cpu_count() or 1) >= n_antennas:
        try:
...
def main():
    ...
    if device.type == 'cpu':
        ...
        # synchronization overhead (measured pathological at 32+ threads).

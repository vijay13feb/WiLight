...
class RepDopplerViT(nn.Module):
    """RepDopplerViT with late fusion. Input (B,1,T,V) one antenna at a time,
    output (B,n_classes) logits; caller sums 4 antenna views then argmax.
    ~182K parameters (in_channels=1)."""
...
def parse_args():
    ...
    p.add_argument('--no_cache_ram', dest='cache_ram', action='store_false',
        help='Disable preloading all windows into RAM (use on low-memory '
             'devices; falls back to unpickling from disk per sample).')
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            torch.save({
                ...
                'n_classes':      n_classes,
                'n_antennas':     n_antennas,
                'channels':       getattr(model, '_channels', (16, 32, 64)),

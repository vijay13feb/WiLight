#!/usr/bin/env python3
"""
train_doppler_vit.py — RepDopplerViT for Doppler spectrogram HAR, late fusion.

Each antenna trains as an independent (1,T,V) sample; at inference the 4
per-antenna logit vectors are summed then argmax'd. Loss is FocalLoss +
supcon_weight * SupConLoss (Khosla et al. 2020) on the GAP embeddings.
Backbone: RepViT + EfficientViT (CVPR 2024); reparameterize() fuses to a
single 3x3 DW conv per block for ONNX export.
"""
...
try:
    ...
    HAS_PSUTIL = False
    print("[warn] psutil not found — RAM reporting skipped.")

ACTIVITY_MAP = {
    'A': 'Walk', 'B': 'Run',  'C': 'Jump',    'D': 'Sitting',
    'E': 'Empty','F': 'Stand','G': 'Wave',     'H': 'Clap',
    'I': 'Laydown','J':'Wipe','K': 'Squat',    'L': 'Stretch',
}
...
# Dataset  —  late-fusion aware

class DopplerDataset(Dataset):
    """Loads (n_antennas, T, V) windows; expand=True yields per-antenna (1,T,V) samples."""
    ...
    def _load_file(self, file_idx: int) -> np.ndarray:
        with open(self.files[file_idx], 'rb') as fp:
            return pickle.load(fp).astype(np.float32)   # (n_ant, T, V)

    # cache n_antennas after first call
    @property
    def n_antennas(self):
        if not hasattr(self, '_n_ant'):
            if self._cache is not None:
                self._n_ant = self._cache[0].shape[0]
            else:
                with open(self.files[0], 'rb') as fp:
                    self._n_ant = pickle.load(fp).shape[0]
        return self._n_ant

    def __len__(self):
        if self.expand:
            return len(self.labels) * self.n_antennas
        return len(self.labels)

    def __getitem__(self, idx):
        if self.expand:
            file_idx = idx // self.n_antennas
            ant_idx  = idx %  self.n_antennas
        else:
            file_idx = idx
            ant_idx  = None

        if self._cache is not None:
            x = self._cache[file_idx]
        else:
            x = self._load_file(file_idx)
        x = torch.from_numpy(x)   # zero-copy view; cached array must not be mutated in place

        if ant_idx is not None:
            x = x[ant_idx : ant_idx + 1]   # slice before augment to avoid augmenting 4x data
...
class SupConLoss(nn.Module):
    ...
    def forward(self, features: Tensor, labels: Tensor) -> Tensor:
        ...
        exp_sim = torch.exp(sim) * (1.0 - eye)      # zero out diagonal
...
class RepBlock(nn.Module):
    def __init__(self, dim: int, expand: int = 2, drop_path: float = 0.0):
        ...
        self.channel_mixer = nn.Sequential(
            ...
            nn.ReLU(),   # XNNPACK fuses Conv2d+BN+ReLU into one kernel on ARM
...
class MBConvBlock(nn.Module):
    """Mobile Inverted Bottleneck: pure XNNPACK path replacing LinearAttnBlock
    (no einsum/permute) — ~5-10x faster on Pi5 CPU, ~1-3% zero-shot accuracy cost."""
    def __init__(self, dim: int, expand: int = 2, drop_path: float = 0.0):
        super().__init__()
        mid = dim * expand
...
# Full model  — in_channels=1 for late fusion

class RepDopplerViT(nn.Module):
    """
    RepDopplerViT with late fusion.

    Input  : (B, 1, T, V)  — one antenna at a time
    Output : (B, n_classes) logits

    At inference the caller feeds 4 antenna views of the same window,
    sums the 4 logit vectors, and takes argmax.
    """

    def __init__(self, n_classes: int,
                 in_channels: int = 1,
                 channels: Tuple[int, int, int] = (16, 32, 64),
                 depth: Tuple[int, int, int]    = (1, 2, 1),
                 heads: int = 4, dim_head: int = 16,
                 mlp_ratio: int = 2, dropout: float = 0.2,
                 drop_path_rate: float = 0.1,
                 attn_mlp_ratio: int = 1,
                 simple_head: bool = False,
                 mobile_stage3: bool = False,
                 hybrid_stage3: bool = True):
        # mobile_stage3/hybrid_stage3 change stage3's parameter names and
        # require retraining; both default to checkpoint-compatible values.
        super().__init__()
        c1, c2, c3 = channels
        d1, d2, d3 = depth
        total = d1 + d2 + d3
        ...
        self._attn_mlp_ratio = attn_mlp_ratio
        ...
        if hybrid_stage3:
            # Three stride-2 downsamples before stage3: 90x25 -> 12x4 (48 tokens,
            # 47x fewer), so both MBConv and LinearAttn run on far less compute.
        ...
        if simple_head:
            # ~20% fewer params; BN stabilises the 96-dim GAP vector.
            # SupConLoss reads features from head[2] (BN output).
            ...
            self.head = nn.Sequential(
                ...
                nn.AdaptiveAvgPool2d(1),
                nn.Flatten(),   # SupCon feature point (192-dim)
                nn.Dropout(dropout),
    ...
    def forward(self, x: Tensor,
        ...
        if self._hybrid_stage3:
            ...
            x = self.stage3(x)
        # SupConLoss features captured at head[2] (BN1d or Flatten output).
        for i, layer in enumerate(self.head):
            x = layer(x)
            if return_features and i == 2:
                feat = x
        if return_features:
...
def print_system_info(model, device):
    ...
    print('─' * 65)
    if HAS_PSUTIL:
        ram_used  = _mb(psutil.Process().memory_info().rss)
    ...
    if device.type == 'cuda':
        ...
        if hasattr(torch.cuda, 'mem_get_info'):   # torch ≥1.10
...
def run_epoch_late_fusion(model, loader, focal_crit, supcon_crit,
                          supcon_w, optimizer, device, train: bool,
                          n_antennas: int = None):
    """Training epoch for late-fusion mode; n_antennas kept for call-site compat, unused."""
    model.train(train)
    ...
    with torch.set_grad_enabled(train):
        for x, y in loader:
            ...
            if train:
                ...
                n_ant = x.shape[1]
                x_exp = x.reshape(B * n_ant, 1, x.shape[2], x.shape[3])
                y_exp = y.repeat_interleave(n_ant)
                logits, feats = model(x_exp, return_features=True)
                focal  = focal_crit(logits, y_exp)
                supcon = supcon_crit(feats, y_exp)
                loss   = focal + supcon_w * supcon
                merged = logits.reshape(B, n_ant, -1).sum(1)
                total_correct += (merged.argmax(1) == y).sum().item()
                total_loss    += loss.item() * B
                total_n       += B
...
@torch.no_grad()
def evaluate_late_fusion(model, loader, device, n_antennas: int = None):
    """Returns (accuracy, macro-F1, weighted-F1, per-class-F1, conf-mat, preds, targets).
    Antenna count auto-detected from x.shape[1]; n_antennas kept for compat, unused."""
    model.eval()
    ...
    for x, y in loader:
        ...
        x_exp    = x.reshape(B * n_ant, 1, x.shape[2], x.shape[3])
        logits   = model(x_exp)
        merged   = logits.reshape(B, n_ant, -1).sum(1)        # (B, C)
        all_preds.extend(merged.argmax(1).cpu().numpy())
...
def plot_confusion_matrix(cm, class_names, path):
    ...
    ax.set_title('Confusion Matrix (row-normalised recall, raw count in grey)')
    plt.tight_layout()
    plt.savefig(path, dpi=150); plt.close()
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Train RepDopplerViT (late fusion) on Doppler spectrogram dataset.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument('dataset_dir',
        help='Scenario folder from creating_train.py  (e.g. output/doppler/S1a)')
    ...
    p.add_argument('--hybrid_stage3',    dest='hybrid_stage3', action='store_true',
        help='Enable hybrid stage3: Downsample(×3)→MBConv→LinearAttn (default).')
    p.add_argument('--no_hybrid_stage3', dest='hybrid_stage3', action='store_false',
        help='Disable hybrid stage3; use plain LinearAttnBlock.')
    p.set_defaults(hybrid_stage3=True)
    p.add_argument('--simple_head',    dest='simple_head', action='store_true',
        help='Use simple head: GAP→BN→Dropout→Linear (default).')
    p.add_argument('--no_simple_head', dest='simple_head', action='store_false',
        help='Revert to original ConvBNAct head.')
    p.set_defaults(simple_head=True)
    p.add_argument('--cache_ram',    dest='cache_ram', action='store_true',
        help='Preload all windows into RAM (default).')
    p.add_argument('--no_cache_ram', dest='cache_ram', action='store_false',
        help='Disable RAM preloading (use on low-memory devices).')
...
def main():
    ...
    for split in ('train', 'val', 'test'):
        ...
        # Training uses expand=True (per-antenna), eval uses expand=False (merged)
    ...
    n_antennas = _load_pkl(base_dir / 'n_antennas.pkl')
    ...
    print('=' * 65)
    print('RepDopplerViT  —  LATE FUSION  (RepViT + EfficientViT, CVPR 2024)')
    print('=' * 65)
    print(f'  Dataset     : {base_dir}')
    print(f'  Device      : {device}')
    print(f'  Antennas    : {n_antennas}  (processed independently, logits merged)')
    ...
    print(f'  Train units : {n_tr * n_antennas}  (each window × {n_antennas} antennas)')
    ...
    print(f'  Parameters  : {count_params(model):,}  (in_channels=1, late fusion)')
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if improved:
            ...
            torch.save({
                ...
                'n_classes':      n_classes,
                'n_antennas':     n_antennas,
                'channels':       getattr(model, '_channels', (24, 48, 96)),
    ...
    acc, f1m, f1w, f1c, cm, preds, targets = evaluate_late_fusion(
        model, loaders['test'], device, n_antennas)

    print('\n' + '=' * 65)
    print('TEST RESULTS  (late fusion: sum of 4 antenna logits)')
    print('=' * 65)
    ...
    sample_exp = sample.reshape(2 * n_antennas, 1, sample.shape[2], sample.shape[3])
    verify_reparameterize(model, sample_exp)
    ...
    with open(res_path, 'w') as fp:
        fp.write(f'Model             : RepDopplerViT (late fusion)\n')

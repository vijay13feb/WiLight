...
Phase 3: few-shot fine-tuning on S2a — see device_finetune.py.
"""
...
from train_doppler_vit import (
    ...
    count_params, evaluate_late_fusion,
...
# Candidate keep_ratios for the automatic Pareto sweep.
# Covers from aggressive pruning (≈3× smaller) to no pruning (1.0).
_DEFAULT_SWEEP_RATIOS: List[float] = [0.25, 0.33, 0.50, 0.67, 0.75, 0.85, 1.00]

# Physical signal parameters — used to derive the real-time latency boundary.
# These match batchprocessing.py: Tc = 1/150 s per Doppler frame, window = 100 frames.
_DOPPLER_FRAME_RATE_HZ: float = 150.0   # STFT Doppler frames per second
_DEFAULT_WINDOW_FRAMES: int   = 100     # default window_length in creating_train.py
...
_DEFAULT_KNOWN_DEVICES: Dict[str, dict] = {
    'pi5_8gb':     dict(arch='aarch64', has_neon=True,  freq_mhz=2400, ram_mb=8192,  gflops=5.3),
    'pi5_4gb':     dict(arch='aarch64', has_neon=True,  freq_mhz=2400, ram_mb=4096,  gflops=5.3),
...
def probe_device() -> dict:
    ...
    arch     = uname.machine          # 'aarch64', 'x86_64', 'armv7l', 'armv6l'
    cpu_model = uname.processor or uname.machine

    # Parse /proc/cpuinfo on Linux for NEON flag and model name
    has_neon = False
    try:
        with open('/proc/cpuinfo') as fh:
            cpuinfo = fh.read()
        # 'asimd' = ARMv8 NEON; 'neon' = ARMv7 NEON
...
def _gflops_for_profile(profile: dict) -> float:
    ...
    # 'architecture' is the newer extract_device_profile.py key name.
    arch = profile.get('arch') or profile.get('architecture', 'x86_64')
    has_neon = profile.get('has_neon', False)
    # Some kernels mis-report NEON on ARMv8 cores that always have ASIMD.
    cpu_model_lc = profile.get('cpu_model', '').lower()
    if any(x in cpu_model_lc for x in ('a55', 'a75', 'a76', 'a77', 'a78',
                                        'a710', 'a715', 'x1', 'x2', 'x3')):
        has_neon = True
...
def benchmark_inference(model: RepDopplerViT,
                        n_antennas: int = 4,
                        window_shape: Tuple[int, int] = (100, 100),
                        n_warmup: int = 10,
                        n_runs: int = 50,
                        device: torch.device = torch.device('cpu')) -> dict:
    """Measure wall-clock inference latency for one 4-antenna window on *device*."""
    model.eval().to(device)
    dummy = torch.zeros(n_antennas, 1, *window_shape, device=device)

    for _ in range(n_warmup):
        _ = model(dummy).reshape(1, n_antennas, -1).sum(1)
    if device.type == 'cuda':
    ...
    for _ in range(n_runs):
        ...
        logits = model(dummy).reshape(1, n_antennas, -1).sum(1)
        if device.type == 'cuda':
            torch.cuda.synchronize()
...
def _print_model_summary(model: RepDopplerViT) -> None:
    ...
    if _hybrid:
        arch_str = 'hybrid_stage3  (Downsample×3 → MBConv → LinearAttn)'
    elif _mobile:
        arch_str = 'mobile_stage3  (MBConv only)'
    else:
        arch_str = 'original       (LinearAttn only)'

    c1 = model.stage1[0].channel_mixer[4].num_features  # post-project BN → dim
    c2 = model.stage2[0].channel_mixer[4].num_features
    if _hybrid:
...
def _cp_repblock(src: RepBlock, dst: RepBlock, idx: Tensor):
    ...
    # Channel mixer: Conv[0] -> BN[1] -> ReLU[2] -> Conv[3] -> BN[4]
    ew       = src.channel_mixer[0].weight.data
    ew_s     = ew[:, idx, :, :]
    hidden_k = dst.channel_mixer[0].weight.data.shape[0]
    ei       = _top_idx(ew_s, hidden_k)
    dst.channel_mixer[0].weight.data.copy_(ew_s[ei])
    _cp_bn(src.channel_mixer[1], dst.channel_mixer[1], ei)
...
def _cp_linear_attn(src: LinearAttnBlock, dst: LinearAttnBlock, idx: Tensor):
    ...
    # FFN hidden size read from dst since attn_mlp_ratio can be 1 or 2.
    _cp_bn(src.ffn[0], dst.ffn[0], idx)
    fw       = src.ffn[1].weight.data
    fw_s     = fw[:, idx, :, :]
    hidden_k = dst.ffn[1].weight.data.shape[0]
    fi       = _top_idx(fw_s, hidden_k)
...
def prune_model(model: RepDopplerViT,
    ...
    # Importance and channel selection
...
def _plot_pareto_curve(results: list, best_idx: int, out_dir: Path,
    ...
    # Classify candidates
    for lat, acc, r in zip(lats, accs, ratios):
...
def incremental_prune_search(
    ...
    if device_prof:
        arch_str = device_prof.get('arch') or device_prof.get('architecture', '?')
        cores    = device_prof.get('cpu_cores') or device_prof.get('logical_threads', '?')
        l2_kb    = device_prof.get('l2_cache_kb')
        l3_kb    = device_prof.get('l3_cache_kb')
        cache_str = (f'  L2={l2_kb} KB' if l2_kb else '') + (f'  L3={l3_kb} KB' if l3_kb else '')
        print(f'  Target device  : {device_prof.get("cpu_model","?")}')
        print(f'  Architecture   : {arch_str}  '
              f'cores={cores}  NEON={device_prof.get("has_neon","?")}')
        if cache_str:
    ...
    for ratio in candidates:
        ...
        acc, f1m, _, _, _, _, _ = evaluate_late_fusion(
            pruned_m, test_loader, device, n_antennas)

        # Device latency from actual MACs of this pruned model
        macs_actual = count_model_macs(
            pruned_m, (1, 1, window_frames, 100))
        macs_m  = macs_actual * n_antennas / 1e6   # total M-MACs for all antennas
    ...
    # Pick the second-smallest keep_ratio within budget for a safety margin.
    if within_budget:
        sorted_wb = sorted(within_budget, key=lambda i: results[i]['keep_ratio'])
        if len(sorted_wb) >= 2:
            best_idx = sorted_wb[1]
        else:
            best_idx = sorted_wb[0]
        sel_drop = results[best_idx]['acc_drop'] * 100
        reason   = (f'second smallest keep_ratio within {max_acc_drop*100:.0f}% accuracy budget'
    ...
    print(f'  Selection criterion : second smallest keep_ratio within {max_acc_drop*100:.0f}% accuracy budget')
...
def parse_args():
    ...
    p.add_argument('dataset_dir', nargs='?', default=None,
        help='S1a dataset folder (e.g. output/doppler/S1a). '
    ...
    search_group.add_argument('--retrain_epochs', type=int, default=15,
    ...
    p.add_argument('--cache_ram',    dest='cache_ram', action='store_true',
        help='Preload all windows into RAM for faster training (default: on).')
    ...
    p.add_argument('--sparsity_epochs', type=int, default=15,
...
def main():
    ...
    with open(base_dir / 'n_antennas.pkl', 'rb') as f:
        n_antennas = pickle.load(f)
    ...
    if args.activities is not None:
        ...
        ckpt_peek = torch.load(base_dir / 'best_model.pth',
        ...
        if ckpt_ld:
            ...
            acts = None   # use all activities in label_dict.pkl
        del ckpt_peek
    ...
    # Derive actual window length from the saved dataset — do not assume 100
    ...
    # Load trained model
    ckpt_path = base_dir / 'best_model.pth'
    ckpt      = torch.load(ckpt_path, map_location=device, weights_only=False)

    # Auto-detect depth from state-dict keys for checkpoints predating 'depth'.
    if 'depth' in ckpt:
        depth = ckpt['depth']
    else:
        import re as _re
        _max = {1: 0, 2: 0, 3: 0}
        for _k in ckpt['model_state']:
            _m = _re.match(r'stage(\d)\.(\d+)\.', _k)
            if _m:
                _s, _i = int(_m.group(1)), int(_m.group(2))
                if _s in _max:
                    _max[_s] = max(_max[_s], _i + 1)
        depth = (_max[1] or 1, _max[2] or 1, _max[3] or 1)
        print(f'  [compat] depth auto-detected from state dict: {depth}')

    # Use the checkpoint's own n_classes so the head weights load correctly.
    ckpt_n_classes = (ckpt.get('n_classes')
                      or ckpt['model_state']['head.4.weight'].shape[0])
    if ckpt_n_classes != n_classes:
        print(f'  [compat] checkpoint has {ckpt_n_classes} output classes '
              f'(trained), filtered to {n_classes} (eval). '
              f'Loading full {ckpt_n_classes}-class weights.')
    ...
    # Older checkpoints don't store 'channels'; read widths from weight shapes.
    if 'channels' in ckpt:
        ...
        if channels != (24, 48, 96):
            print(f'  [compat] channels auto-detected from state dict: {channels}')
    model = RepDopplerViT(n_classes=ckpt_n_classes, in_channels=1,
    ...
    bl_acc, bl_f1, _, _, _, _, _ = evaluate_late_fusion(
        model, test_loader, device, n_antennas)
    ...
    if device_prof and gflops_dev:
        ...
        cores_th  = device_prof.get('cpu_cores') or device_prof.get('logical_threads', '?')
        pt_ver    = device_prof.get('pytorch_version', '')
        pt_str    = f'  PyTorch {pt_ver}' if pt_ver else ''
        print(f'  Device          : {device_prof.get("cpu_model","?")} @ '
              f'{device_prof.get("cpu_freq_max_mhz","?")} MHz  '
              f'cores={cores_th}  '
              f'NEON={device_prof.get("has_neon","?")}')
        if pt_str:
            print(f'  Runtime         :{pt_str}')
    ...
    print(f'      --doppler_dir <doppler_dir> --scenario S2a --activities {args.activities}')

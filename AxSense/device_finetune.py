...
def _torch_load(path, map_location=None):
    # weights_only was added in PyTorch 1.13; fall back for 1.12 and older.
    try:
        return torch.load(path, map_location=map_location, weights_only=False)
    except TypeError:
        return torch.load(path, map_location=map_location)

# Radar / windowing constants
SAMPLING_HZ    = 150    # radar packets per second  (1 frame = 1/150 s ≈ 6.67 ms)
WINDOW_FRAMES  = 100    # frames per window  →  100/150 = 0.667 s of real radar data
WINDOW_STRIDE  = 10     # stride between windows  →  10/150 = 0.067 s step
...
class MemDataset(Dataset):
    """
    expand=True  : one (1,T,V) per-antenna frame per window (fine-tuning).
    expand=False : full (n_ant,T,V) window (late-fusion eval).

    Tensors are pre-converted once in __init__ so __getitem__ never allocates.
    """
    def __init__(self, windows: List[np.ndarray], labels: List[int],
        ...
        self.expand  = expand
        self._n_ant  = windows[0].shape[0] if windows else 4
        # Pre-convert to float32 tensors once — avoids per-sample numpy allocation
        self._tensors = [torch.from_numpy(w.astype(np.float32)) for w in windows]
    ...
    def __getitem__(self, idx):
        ...
        # Select antenna before clone/augment to avoid cloning unneeded data.
        x = self._tensors[fi]
        if ai is not None:
            x = x[ai : ai + 1]   # (1, T, V)
...
def load_scenario_windows(doppler_dir: Path,
    ...
    """Load ALL Doppler windows for a given scenario (e.g. S2a, S1a).
    Globs signal_{scenario}_{act}_*.npz files from doppler_dir."""
    ...
    for act in activities:
        ...
        for fpath in sorted(doppler_dir.glob(f'signal_{scenario}_{act}_*.npz')):
            m = re.match(rf'signal_{scenario}_[A-Z]_(\d+)\.npz', fpath.name)
            if m:
        ...
        raw   = [np.load(ant_files[i])['doppler'].astype(np.float32)
                 for i in ant_idxs]
        min_f = min(d.shape[0] for d in raw)
        mc    = np.stack([d[:min_f] for d in raw], axis=0)
        n_win = (min_f - window_length) // stride_length + 1
        for k in range(n_win):
            s = k * stride_length
            windows.append(mc[:, s : s + window_length, :])
            labels.append(label_dict[act])
        print(f'  {scenario} [{act}]: {min_f} frames → {n_win} windows '
              f'({len(ant_idxs)} antennas)')
...
def cap_windows_per_class(windows: List[np.ndarray], labels: List[int],
    ...
    for lb in sorted(class_indices):
        ...
        if len(idxs) > max_per_class:
            ...
            idxs = [idxs[j] for j in sub]   # sub are positions within idxs, not global indices
...
def few_shot_finetune(model: RepDopplerViT,
    ...
    supcon_weight re-clusters S2a embeddings to fix the Wave/Jump domain-shift
    overlap; when mixup_alpha > 0 too, SupCon uses the clean batch and Focal+CE
    get a second forward pass on the mixed batch. reparameterize() fuses the
    frozen stages for faster inference. Returns (fine-tuned model, stats dict).
    """
    # 1. Reparameterize frozen stages
    ...
    if unfreeze_stage3:
        if _hybrid:
            for p in model.down_pre.parameters():
                p.requires_grad = True
            for p in model.stage3_local.parameters():
                p.requires_grad = True
            for p in model.stage3_attn.parameters():
                p.requires_grad = True
        else:
    ...
    # SupCon needs a trainable backbone — frozen features can't be re-clustered.
    use_supcon = supcon_weight > 0.0 and unfreeze_stage3

    if verbose:
        print(f'    Trainable: {trainable:,}  |  Frozen: {frozen:,}  '
              f'|  stage3={unfreeze_stage3}'
              + ('  |  BN1d=eval' if freeze_bn else '')
              + (f'  |  supcon w={supcon_weight} τ={supcon_temp}' if use_supcon else '')
              + (f'  |  mixup α={mixup_alpha}' if mixup_alpha > 0 else ''))
    ...
    if verbose:
        print(f'    FLOPs/epoch={ft_flops_ep/1e6:.1f}M  '
              f'max_ep={epochs}  patience={patience}  warmup={warmup_epochs}')
    ...
    if use_feat_cache:
        ...
        feat_loader = DataLoader(
            ...
            batch_size=min(batch_size, len(feats_all)), shuffle=True, num_workers=0)
    ...
    loader = DataLoader(support_ds,
                        batch_size=min(batch_size, len(support_ds)),
    ...
    # Re-clusters S2a embeddings: pulls same-class together, pushes others apart.
    supcon = SupConLoss(temperature=supcon_temp) if use_supcon else None

    # 4. Optimizer — differential LR: stage3 gets full lr, head gets lr/2
    if unfreeze_stage3:
        if _hybrid:
            s3_params = (list(model.down_pre.parameters()) +
                         list(model.stage3_local.parameters()) +
                         list(model.stage3_attn.parameters()))
        else:
            s3_params = list(model.stage3.parameters())
        param_groups = [
    ...
    warmup_sched = torch.optim.lr_scheduler.LinearLR(
        optim, start_factor=0.1, end_factor=1.0, total_iters=warm)
    cosine_sched = torch.optim.lr_scheduler.CosineAnnealingLR(
        optim, T_max=max(epochs - warm, 1), eta_min=1e-6)
    sched = torch.optim.lr_scheduler.SequentialLR(
        optim, schedulers=[warmup_sched, cosine_sched], milestones=[warm])
    ...
    for ep in ep_bar:
        ...
        for x, y in active_loader:
            ...
            if use_feat_cache:
                ...
                # Clean forward for SupConLoss (needs real labels, not mixed)
                logits, feats = model(x, return_features=True)
                sc_loss = supcon(feats, y)
                if use_mixup:
                    # Second forward on mixed batch for Focal+CE
                    x_m, y_a, y_b, lam = _mixup_batch(x, y, mixup_alpha)
                    logits_m = model(x_m)
    ...
    ft_flops_actual = ft_flops_ep * actual_eps
    ft_stats = {
...
def eval_query(model: RepDopplerViT,
               query_ds: MemDataset,
               device: torch.device,
               n_antennas: int) -> Tuple[float, float, float, np.ndarray, float, int]:
    """Late-fusion eval.
    Returns (acc, f1_macro, f1_weighted, cm, model_inf_s, n_windows).
    model_inf_s: pure model forward-pass time only.
    Excludes: DataLoader fetch, .to(device), numpy<->tensor conversion, metrics."""
    model.eval()
    # batch_size=1 matches deployment: windows arrive sequentially from the radar.
    ...
    for x, y in batch_bar:
        B      = x.shape[0]
        x_exp  = x.reshape(B * n_antennas, 1, x.shape[2], x.shape[3]).to(device)
        if device.type == 'cuda':
            torch.cuda.synchronize(device)
        _t  = time.perf_counter()
        out = model(x_exp)
    ...
    cm = confusion_matrix(targets, preds, labels=sorted(set(targets)))
...
def profile_single_window(model: nn.Module,
    ...
    Returns (latency_ms, cpu_rss_peak_delta_mb, peak_gpu_mb)."""
    model.eval()
    ...
    if device.type == 'cuda':
        torch.cuda.synchronize(device)
        torch.cuda.reset_peak_memory_stats(device)
    # Latency: no threading overhead so timer is accurate
    ...
    return latency_ms, cpu_rss_delta, gpu_mb
...
def zero_shot_eval(model, all_windows, all_labels,
    ...
    acc, f1_mac, f1_wt, cm, inf_model_s, n_win = eval_query(model, q_ds, device, n_antennas)
    ...
    sw_lat_ms, sw_rss_mb, sw_gpu_mb = profile_single_window(model, device, n_antennas)
...
def run_fewshot_experiment(pruned: RepDopplerViT,
    ...
    Stage3 auto-unfrozen for K >= unfreeze_k (default 5, was 10).
    ...
    print(f'\n── Phase 3: Few-Shot Fine-Tuning  (trials={n_trials}, '
          f'γ={gamma}, lr={lr}, bs={batch_size}, max_ep={finetune_epochs}) ──')
    print(f'   warmup={warmup_epochs}ep  mixup_α={mixup_alpha}  '
          f'stage3_unfreeze_k≥{unfreeze_k}')
    ...
    for K in k_shots:
        ...
        do_stage3              = K >= unfreeze_k   # was K >= 10
        ...
        if K >= windows_per_class:
            print(f'  [skip] K={K}: {windows_per_class} windows/class — '
                  ...
                  f'Use --max_windows > {K} or reduce --k_shots.')
            continue
        ...
        for trial in trial_bar:
            ...
            print(f'    trial {trial+1} (seed={seed}): '
                  ...
                  f'(query={n_q} win)  '
                  f'ep={ft_stats["n_epochs"]}/{finetune_epochs}  '
        ...
        sw_lat, sw_rss, sw_gpu = profile_single_window(m, device, n_antennas)
        ...
        print(f'       FLOPs/epoch   : {ft_flops_ep/1e6:.1f} M')
        actual_eps_mean = ft_stats['n_epochs'] if ft_stats else finetune_epochs
        print(f'       FLOPs total   : {ft_flops_tot_trial/1e9:.2f} G  '
              f'({actual_eps_mean} epochs actual, max={finetune_epochs})')
...
def write_results(path: Path,
    ...
    lines += [sep,
              ...
              f'  {"K":>6}  {"Acc":>14}  {"F1mac":>14}  {"G:Wave-Rec":>12}'
              f'  {"Support":>8}  {"Query":>7}'
    ...
    for K, r in results.items():
        g_rm, g_rs = r['per_class_recall'].get('G', (0.0, 0.0))
        nq_  = r["n_query"]
...
def parse_args():
    ...
    p.add_argument('--patience', type=int, default=10,
        help='Early-stopping patience: stop if loss does not improve for this '
             'many epochs.  Lower = faster but may stop before convergence.')
    p.add_argument('--lr', type=float, default=5e-4,
        help='Learning rate for trainable parameters.')
    p.add_argument('--batch_size', type=int, default=32,
        help='Mini-batch size for fine-tuning.')
    p.add_argument('--gamma', type=float, default=2.0,
        help='Focal loss γ.  γ=2 gives best overall accuracy; γ=3 focuses '
             'harder on confusable pairs (G:Wave↔C:Jump) but lowers overall acc.')
    # Accuracy-improvement options (target: 90% at K=10-30)
    p.add_argument('--warmup_epochs', type=int, default=5,
        help='LR warm-up epochs before cosine annealing (default 5).  '
             'Protects pretrained features from large early gradient steps.')
    p.add_argument('--mixup_alpha', type=float, default=0.2,
        help='Mixup Beta parameter (default 0.2).  Creates virtual '
             'Wave/Jump boundary samples from the support set.  Set 0 to disable.')
    p.add_argument('--unfreeze_k', type=int, default=5,
        help='Unfreeze stage3 (global attention) when K >= this value '
             '(default 5, original was 10).  Lower = stage3 fine-tunes '
             'on more K values, improving Wave pattern adaptation.')
    p.add_argument('--supcon_weight', type=float, default=0.1,
        help='Weight of SupConLoss added during fine-tuning (default 0.1). '
             'Re-clusters S2a embeddings: pulls same-class Wave features together '
             'and pushes Wave away from Jump/Clap in the 96-dim feature space. '
             'Set 0 to disable.  Tip: increase to 0.3 if Wave recall < 70%%.')
    p.add_argument('--supcon_temp', type=float, default=0.07,
        help='SupConLoss temperature (default 0.07, matches S1a training). '
             'Lower = sharper clusters but harder to optimise with small K.')
    ...
    p.add_argument('--window_frames', type=int, default=360,
        help='Doppler frames per window — must match window_length used in '
             'creating_train.py (default 360).')
    p.add_argument('--window_stride', type=int, default=10,
        help='Stride between windows — must match stride_length used in '
             'creating_train.py (default 10).')
    p.add_argument('--max_windows', type=int, default=None,
        help='Cap each activity to this many windows when loading the scenario '
             'dataset (evenly spaced).  Default: use all windows.  '
             'Example: --max_windows 100 loads 100 windows/activity.')
    return p.parse_args()
...
def main():
    ...
    depth          = ckpt.get('depth', (1, 1, 1))
    n_classes      = ckpt['n_classes']
    n_antennas     = ckpt['n_antennas']
    label_dict     = ckpt['label_dict']

    # Default: derive activities from checkpoint label_dict to match the model.
    ...
    # Defaults cover checkpoints that predate these fields.
    ...
    if args.inference_model:
        ...
        ft_dep  = ft_ckpt.get('depth', (1, 1, 1))
        ...
        sw_lat, sw_rss, sw_gpu = profile_single_window(ft_model, device, ft_na)
        print(f'  Single-window latency : {sw_lat:.3f} ms  '
              f'peak RAM {sw_rss:.1f} MB')
        if HAS_PLOT:
    ...
    hdr = (f'  {"K":>6}  {"Acc(mean±std)":>17}  {"F1mac":>17}  '
           f'{"G:Wave-Recall":>15}  {"Support":>8}  {"Query":>7}  '

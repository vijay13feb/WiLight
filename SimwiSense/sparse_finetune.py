...
# MACs (multiply-accumulate ops) for one full single-antenna inference at
# channels=(24,48,96): Stage1≈50M + Stage2≈72M + Stage3≈121M + Head≈12M = 255M
_BASE_MACS_M   = 198.0            # MACs per antenna at keep_ratio=1.0  (depth 2,2,2)
_FLOPS_EXPONENT = 1.5             # MACs ∝ keep_ratio^1.5 (PW convs are C², DW are C)
...
# SimWiSense: 500 packets/s, SLIDING=10 → Doppler frame rate = 50 Hz.
# Window = 100 frames → 100/50 = 2.0 s of real radar data.
_DOPPLER_FRAME_RATE_HZ: float = 50.0    # STFT Doppler frames per second
...
def _cp_repblock(src: RepBlock, dst: RepBlock, idx: Tensor):
    ...
    # Expand PW  (C → C*r) — derive expansion ratio r from source weight shape
    ew      = src.channel_mixer[1].weight.data         # (C*r, C, 1, 1)
    C_src   = ew.shape[1]                              # original C before pruning
    r       = ew.shape[0] // C_src                     # mlp_ratio (1, 2, …)
    H       = K * r                                    # hidden size after pruning
    ew_s    = ew[:, idx, :, :]                         # (C*r, K, 1, 1)
    ei      = _top_idx(ew_s, H)                        # (H,) sorted
    dst.channel_mixer[1].weight.data.copy_(ew_s[ei])

    # Contract PW  (C*r → C) — select same H hidden rows, K output rows
    cw   = src.channel_mixer[3].weight.data            # (C, C*r, 1, 1)
...
def _cp_linear_attn(src: LinearAttnBlock, dst: LinearAttnBlock, idx: Tensor):
    ...
    # Q, K, V projections: (inner, C, 1,1) → (inner, K, 1,1)
    for name in ('q', 'k', 'v'):
        src_c = getattr(src, name)
        dst_c = getattr(dst, name)
        dst_c.weight.data.copy_(src_c.weight.data[:, idx])

    # Attention output projection: (C, inner, 1,1) → (K, inner, 1,1)
    dst.attn_proj[0].weight.data.copy_(src.attn_proj[0].weight.data[idx])
    _cp_bn(src.attn_proj[1], dst.attn_proj[1], idx)

    # FFN: BN(C) → Conv(C→C*r) → GELU → Conv(C*r→C) → BN(C)
    _cp_bn(src.ffn[0], dst.ffn[0], idx)
    fw      = src.ffn[1].weight.data            # (C*r, C, 1, 1)
    C_src   = fw.shape[1]                       # original C
    r       = fw.shape[0] // C_src             # mlp_ratio from source
    H       = K * r                             # hidden size after pruning
    fw_s    = fw[:, idx, :, :]                  # (C*r, K, 1, 1)
    fi      = _top_idx(fw_s, H)                 # (H,) sorted hidden idx
    dst.ffn[1].weight.data.copy_(fw_s[fi])
    cw   = src.ffn[3].weight.data               # (C, C*r, 1, 1)
...
def _cp_mbconv(src: MBConvBlock, dst: MBConvBlock, idx: Tensor):
    ...
    # Channel expand: pick best hidden_k rows by L2 norm
    ew       = src.pw_exp[0].weight.data              # (mid, C, 1, 1)
    ew_s     = ew[:, idx, :, :]                       # (mid, K, 1, 1)
    hidden_k = dst.pw_exp[0].weight.data.shape[0]     # K*expand
    ei       = _top_idx(ew_s, hidden_k)
    dst.pw_exp[0].weight.data.copy_(ew_s[ei])
    _cp_bn(src.pw_exp[1], dst.pw_exp[1], ei)

    # Channel project: same hidden selection → idx output
    cw = src.pw_proj[0].weight.data                   # (C, mid, 1, 1)
...
def _cp_head(src_head: nn.Sequential, dst_head: nn.Sequential, in_idx: Tensor):
    ...
    if isinstance(src_head[2], nn.BatchNorm1d):
        ...
        hw   = src_head[0][0].weight.data    # (2*C3, C3, 1, 1)
        hw_s = hw[:, in_idx, :, :]           # (2*C3, K, 1, 1)
        hi   = _top_idx(hw_s, 2 * K)         # top 2K output rows by L2 norm
        dst_head[0][0].weight.data.copy_(hw_s[hi])
        _cp_bn(src_head[0][1], dst_head[0][1], hi)
        # head[4] = Linear(2*K→n_classes): select the same 2K input columns
...
def incremental_prune_search(
    ...
    t_window_ms = _window_collection_ms()
    ...
    print(f'  T_window       : {t_window_ms:.0f} ms  (100 frames ÷ 150 Hz)')
    ...
    print(f'  MACs/window : {macs_total_m:.1f} M  ({n_antennas} antenna)')
    ...
    for ratio in candidates:
        ...
        # Accuracy drop vs original model
...
def parse_args():
    ...
    p.add_argument('--activities', default=None,
        help='Comma-separated activity letters, e.g. A,B,C. '
    ...
    p.add_argument('--model_dir', default=None, metavar='PATH',
        help='Folder containing best_model.pth (default: same as dataset_dir). '
             ...
             'than the dataset, e.g. OUTPUT_BASE != INPUT_BASE.')
...
def main():
    ...
    model_depth  = ckpt.get('depth', _infer_depth(ckpt['model_state']))
    n_classes_ckpt = ckpt.get('n_classes', n_classes)
    # simple_head: default True (new default); False only for old pre-flag checkpoints.
    ...
    print(f'  Model depth    : {model_depth}  (from checkpoint)')
    ...
    # Channel widths: stored by newer train_doppler_vit.py checkpoints; for
    # older ones read them from the weight shapes so non-default widths load.
    if 'channels' in ckpt:
    ...
    model = RepDopplerViT(n_classes=n_classes, in_channels=1,
                          channels=channels, depth=model_depth,
                          mlp_ratio=mlp_ratio,
    ...
    bl_macs_per_ant = count_model_macs(model, (1, 1, 100, 100))
    ...
    bl_t_window = _window_collection_ms()
    ...
    macs_pruned = report_model_flops(pruned, n_antennas, window_shape=(_DEFAULT_WINDOW_FRAMES, 100))

    # Optional post-hoc benchmark
    if args.benchmark:
        print(f'\n── Inference Benchmark  (CPU, {n_antennas} antenna) ──')
    ...
    torch.save({
        ...
        'window_frames':    _DEFAULT_WINDOW_FRAMES,
    }, save_path)
    ...
    if device_prof and gflops_dev:
        n_cls   = len(acts)
        t_win_s = _DEFAULT_WINDOW_FRAMES / _DOPPLER_FRAME_RATE_HZ
        ...
        print()
        print(f'  Inference (per window = {_DEFAULT_WINDOW_FRAMES}/{_DOPPLER_FRAME_RATE_HZ:.0f}Hz'
